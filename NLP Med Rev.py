...
def normalizer(revs): #### Cleaning Reviews
    soup = BeautifulSoup(revs, 'lxml')   # removing HTML encoding such as ‘&amp’,’&quot’
...
def SVC_Save(data, targets, tfidf):
    ...
    pd.DataFrame.from_dict(data=dict([word, i] for i, word in enumerate(tfidf.get_feature_names())), orient='index').to_csv('ReviewsVocabulary_SVC.csv', header=False)
...
def NBC_Save(data, targets, tfidf):
    tfidf.fit(data) # learn vocabulary of entire data
    data_tfidf = tfidf.transform(data)
    pd.DataFrame.from_dict(data=dict([word, i] for i, word in enumerate(tfidf.get_feature_names())), orient='index').to_csv('ReviewsVocabulary_NBC.csv', header=False)
...
def main():
    ...
    #df = pd.read_csv("MedReviews.csv",  quoting=csv.QUOTE_NONE, encoding = "ISO-8859-1")
    df = pd.read_csv("MedReviews.csv",  encoding='ISO-8859-1',error_bad_lines= False, engine = 'python' )
    #df.drop(index=2115, axis = 0)
    print(df.head())
    pd.set_option('display.max_colwidth', -1) # Setting this so we can see the full content of cells
    ### converting categorical values to numerical values
    df['Rating'] = df['Rating'].map({'Low':0, 'High':1})
    #### Normalizing Reviews
    ...
    print("Printing top 5 rows of dataframe showing original and cleaned revs....")
    ...
    cleaned_data = pd.read_csv("cleaned_Reviews.csv", encoding = "ISO-8859-1")
